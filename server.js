...
const io = socketIo(app); // Correct way to initialize Socket.IO
...
function handler(req, resp) {
    ...
    if (r_url.pathname.substring(1) === "getport") {
        ...
        const clientui = fs.readFileSync("chess.html");
        resp.write(clientui);
        resp.end();
    } else {
        const filename = r_url.pathname.substring(1);
        let type;
...
io.on('connection', function (sk) {
    ...
    sk.on('setup', function (data) {
        // Handle player setup
        skColor = data.color || 'U'; // Default to 'U' if no color specified

        // Player disconnect handling
        sk.on('disconnect', function () {
            if (queue[skColor]) {
                const index = queue[skColor].indexOf(sk);
                if (index > -1) {
        ...
        if (skColor === 'W') {
            if (queue['B'].length > 0) {
                b = queue['B'].shift();
                GameList.addGame(sk, b); // Start new game
            } else {
                queue['W'].push(sk); // Add to waiting queue
            }
        } else if (skColor === 'B') {
            if (queue['W'].length > 0) {
                w = queue['W'].shift();
                GameList.addGame(w, sk); // Start new game
            } else {
                queue['B'].push(sk); // Add to waiting queue
            }
        } else { 
            // Handle undefined color cases
            if (queue['W'].length > 0) {
                w = queue['W'].shift();
                GameList.addGame(w, sk); // Start new game
                ...
                queue['U'].push(sk); // Add to undefined color queue
            }
        }
    });
});

// GameList and Game classes remain unchanged...
